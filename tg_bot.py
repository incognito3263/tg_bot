...
bot = Bot(token='5685167792:AAGGKq3_E4SYrvsfc1yLiFWuhTiXZ5_48NI')
dp = Dispatcher(bot)


@dp.message_handler(commands=['start', ])
async def send_welcome(message: types.Message):
    await message.reply('Вас приветствует Telegram-бот компании MoonLight', reply_markup=kb.start_kb)
...
async def echo(message: types.Message):
    ...
    if sent_answer is False:
        if message.text == '🏞 Каталог':
            ...
            text = '📍 Адрес:\n          ул. Amir Temur 270 ( Ориентир: Nimadir )\n\n'
            text += '☎️ Телефон:\n          +998 (97) 914-64-79\n\n🖥 Сайт:\nhttps://moon.uz'
            await message.answer(text)
